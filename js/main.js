...
function initDeviceGallery() {
  ...
  const thumbsWrap = document.querySelector("[data-dev-thumbs]");
  if (!root || !thumbsWrap) return;

  const main = root.querySelector("[data-dev-main]");
  const thumbs = Array.prototype.slice.call(thumbsWrap.querySelectorAll(".thumb"));
  const prev = root.querySelector("[data-dev-prev]");
  const next = root.querySelector("[data-dev-next]");
  let idx = 0;
  ...
  const render = (i) => {
    ...
    // 활성 썸네일의 실제 이미지를 모니터에 반영 (data-full 있을 때)
    const full = thumbs[idx].getAttribute("data-full");
    if (full) {
      main.innerHTML =
        '<img src="' + full + '" alt="" style="width:100%;height:100%;object-fit:cover;">';
  ...
  thumbs.forEach((t, n) => t.addEventListener("click", () => render(n)));
...
function initBeforeAfter() {
  ...
  const move = (clientX) => {
    if (dragging) setPos(pointerToPct(clientX));
  };
  ...
  dots.forEach((d, i) => {
    d.addEventListener("click", () => {
      ...
      setPos(presets[i] != null ? presets[i] : 50);
    });
  });
